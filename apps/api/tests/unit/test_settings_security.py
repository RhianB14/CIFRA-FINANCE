...
def make_settings(**overrides: Any) -> Settings:
    defaults: dict[str, Any] = {
        ...
        "totp_encryption_key": "unit-test-totp-key-0123456789abcdef0123456789abcdef",
...
def test_rejects_empty_jwt_signing_key() -> None:
    settings = make_settings(jwt_signing_key="", totp_encryption_key=None)
...
def test_rejects_missing_totp_encryption_key() -> None:
    settings = make_settings(totp_encryption_key=None)
    with pytest.raises(RuntimeError) as raised:
        ensure_secure_configuration(settings)
...
def test_test_environment_is_exempt() -> None:
    settings = make_settings(jwt_signing_key="", totp_encryption_key=None, environment="test")
    ensure_secure_configuration(settings)


def test_development_with_empty_keys_raises() -> None:
    settings = make_settings(
        environment="development", jwt_signing_key="", totp_encryption_key=None
    )
    with pytest.raises(RuntimeError) as raised:
