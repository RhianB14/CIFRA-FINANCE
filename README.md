...
Access token dura 15 minutos (HS256, claims `iss`, `aud`, `typ`, `jti`, `sub`, `sv` — inteiro estrito ≥ 1, sem valor padrão). Refresh dura 30 dias, é rotacionado a cada uso e só existe no banco como SHA-256. Reuso de refresh revogado revoga toda a família e invalida as sessões do usuário via versão de sessão durável no PostgreSQL publicada no Redis após o commit; a API responde 503 se o Redis estiver indisponível (fail-closed). Login de usuário inativo é indistinguível de credenciais inválidas e contas desativadas não autenticam, não rotacionam refresh e não consomem challenges. Senhas em Argon2id com rehash transparente no login; o registro consulta o HIBP k-anonymity quando `HIBP_ENABLED=true` e rejeita senhas vazadas; falha do HIBP com política fail-closed responde 503 sem persistir nada. Segredos TOTP são criptografados com Fernet (`TOTP_ENCRYPTION_KEY`) e os backup codes são HMAC-SHA256 com pepper independente (`BACKUP_CODE_PEPPER`); chave JWT, chave Fernet e pepper são validados no startup em tamanho, formato e independência mútua. A janela TOTP é simétrica (±1 janela por padrão, `TOTP_DRIFT_SECONDS // TOTP_PERIOD`) com antirreplay por passo. Desativar o 2FA exige senha e segundo fator. Ativar ou desativar o 2FA encerra as sessões anteriores.

### Segurança de base (F1.5)

| Endpoint | Função |
|---|---|
| `POST /auth/password-recovery` | Solicita token de recuperação (200 indistinguível para e-mail conhecido ou não) |
| `POST /auth/password-reset` | Consome o token e redefine a senha (400 tipado se inválido/expirado) |

Rate limiting por janela fixa no Redis (`cifra:ratelimit:*`): registro 3/min e login 5/min por identidade, resposta 429 com `Retry-After`; headers de proxy só são confiáveis com `TRUST_PROXY_HEADERS=true` e lista explícita em `TRUSTED_PROXIES`. Account lockout com backoff exponencial (`cifra:lock:*`): 5 falhas travam o login por 15 minutos mesmo com senha correta, com resposta indistinguível e fail-open se o Redis cair. Toda a superfície de auth grava eventos tipados em `audit_events` (append-only por trigger e RLS, migração 0002), com valores sensíveis substituídos por `[REDACTED]`. A RLS é efetiva: a API conecta como `cifra_app` (NOLOGIN, NOBYPASSRLS) e o escopo do usuário é re-aplicado a cada transação; `POST /auth/register`, `POST /auth/login` e a recuperação de senha operam em modo bypass antes da autenticação. Requests recebem schemas Pydantic com `extra="forbid"` (anti mass assignment). A recuperação de senha emite tokens de 32 bytes consumidos uma única vez via `GETDEL`, armazenados apenas como SHA-256 com TTL de 15 minutos; a entrega usa Resend quando `PASSWORD_RESET_RESEND_ENABLED=true` (exige `RESEND_API_KEY` e `RESEND_FROM` em produção) e fica em no-op com log caso contrário. A API registra `PASSWORD_RESET_REQUESTED` em toda solicitação e o reset concluído rotaciona a senha, revoga os refresh tokens e bumpa a versão de sessão. O CORS opera em whitelist explícita (`CORS_ALLOWED_ORIGINS`) com `allow_methods` restritos e validação fail-closed no startup. O web aplica headers de segurança em todas as rotas: CSP sem `unsafe-eval`/`unsafe-inline` em script e estilo, HSTS com `includeSubDomains; preload`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: same-origin` e `Permissions-Policy` mínima.
